...
const renderPeople = (people) => {
  renderData("people", people, function(person) {
    return `
...
const renderFilms = (films) => {
  renderData("films", films, (film) => {
    return `
        <b>${film.title}</b>
        <br />
        Released On ${
          ["Su", "Mo", "Tues", "Wed", "Thurs", "Friday", "Sat"][
            new Date(film.release_date).getDay()
          ]
        } ${film.release_date}
      `;
  });
...
const renderVehicles = (vehicles) => {
  renderData("vehicles", vehicles, (vehicle) => {
    return `
        ${vehicle.name}
        <br />
...
const renderStarships = (starships) => {
  renderData("starships", starships, (starship) => {
    return `
        ${starship.name}
        <br />
